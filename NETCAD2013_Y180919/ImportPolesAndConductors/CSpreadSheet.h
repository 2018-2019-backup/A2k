	...
	else // if file is a text delimited file
	{
		if (Open())
		{
			if ((m_bBackup) && (m_bAppend))
			{
				...
				m_stempSql.Format(L"%s.bak", m_sFile);
				m_sFile = m_stempSql;
				if (!Commit())
				{
					m_bBackup = false;
...
bool CSpreadSheet::DeleteSheet()
{
	if (m_bExcel)
	{
		...
		{
			m_sLastError = L"Error deleting sheet\n";
			return false;
		}
	}
	else
...
bool CSpreadSheet::DeleteSheet(CString SheetName)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		SheetName = L"[" + SheetName + L"$A1:IV65536]";
		m_stempSql.Format (L"DROP TABLE %s", SheetName);
		try
		{
...
bool CSpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	...
	if (row == 1)
	{
		if (m_bExcel) 
		{
			...
			{
				m_sLastError = L"Number of columns in new header row cannot be less than the number of columns in previous header row";
				return false;
			}
	...
	else
	{
		if (m_bExcel)
		{
			...
			{
				m_sLastError = L"No header row. Add header row first\n";
				return false;
			}
		}
	}
	...
	if (m_bExcel) // For Excel spreadsheet
	{
		...
		{
			m_sLastError = L"Number of columns to be added cannot be greater than the number of fields\n";
			return false;
		}
	}
	...
	for (int i = 0; i < RowValues.GetSize(); i++)
	{
		...
		{
			m_stempSql.Format(L"\"%s\"%s", RowValues.GetAt(i), m_sSeparator);
			m_stempString += m_stempSql;
		}
		else // Last column
		{
			m_stempSql.Format(L"\"%s\"", RowValues.GetAt(i));
			m_stempString += m_stempSql;
		}
	}
...
bool CSpreadSheet::AddCell(CString CellValue, short column, long row)
{
	if (column == 0)
	{
		m_sLastError = L"Column cannot be zero\n";
		return false;
	}
	...
	if (m_bExcel) // For Excel spreadsheet
	{
		...
		{
			m_sLastError = L"Cell column to be added cannot be greater than the number of fields\n";
			return false;
		}
	}
	else // For text delimited spreadsheet
	...
	for (int j = 1; j <= m_dTotalColumns; j++)
	{
		if (j != m_dTotalColumns) // Not last column
		{
			if (j != column)
			{
				if (m_bExcel)
				{
					m_stempSql.Format(L"\" \"%s", m_sSeparator);
				}
				else
				{
					m_stempSql.Format(L"\"\"%s", m_sSeparator);
			...
			{
				m_stempSql.Format(L"\"%s\"%s", CellValue, m_sSeparator);
				m_stempString += m_stempSql;
			}
		}
		else // Last column
		{
			...
			{
				m_stempSql.Format(L"\"%s\"", CellValue);
				m_stempString += m_stempSql;
			}
		}
	}	
...
bool CSpreadSheet::ReplaceRows(CStringArray &NewRowValues, CStringArray &OldRowValues)
{
	...
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
		m_stempSql.Format(L"UPDATE [%s] SET ", m_sSheetName);
		for (int i = 0; i < NewRowValues.GetSize(); i++)
		{
			m_stempString.Format(L"[%s]='%s', ", m_aFieldNames.GetAt(i), NewRowValues.GetAt(i));
			m_stempSql = m_stempSql + m_stempString;
		}
		m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
		m_stempSql = m_stempSql + L" WHERE (";
		for (int j = 0; j < OldRowValues.GetSize()-1; j++)
		{
			m_stempString.Format(L"[%s]='%s' AND ", m_aFieldNames.GetAt(j), OldRowValues.GetAt(j));
			m_stempSql = m_stempSql + m_stempString;
		}
	...
	{
		m_sLastError = L"Function not available for text delimited file\n";
		return false;
	}
}
...
bool CSpreadSheet::ReadRow(CStringArray &RowValues, long row)
{
	...
	if (row <= m_aRows.GetSize())
	{
		...
		else
		{
			...
			else	// Treat spreadsheet as having one column
			{
				...
				int quoteBegPos = m_stempString.Find(L'\"');
				int quoteEndPos = m_stempString.ReverseFind(L'\"');
	...
	m_sLastError = L"Desired row is greater than total number of rows in spreadsheet\n";
	return false;
}
...
bool CSpreadSheet::ReadColumn(CStringArray &ColumnValues, short column)
{
	...
	for (int i = 1; i <= m_aRows.GetSize(); i++)
	{
		...
		{
			m_dCurrentRow = tempRow;
			m_sLastError = L"Error reading row\n";
			return false;
		}
	}
	m_dCurrentRow = tempRow;
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		if (m_bAppend)
		{
			...
			m_stempString= L"[" + m_sSheetName + "$A1:IV65536]";
			m_stempSql.Format (L"DROP TABLE %s", m_stempString);
			try
			{
				m_Database->ExecuteSQL(m_stempSql);
			}
			...
			// Create new sheet
			m_stempSql.Format(L"CREATE TABLE [%s$A1:IV65536] (", m_sSheetName);
			for (int j = 0; j < m_aFieldNames.GetSize(); j++)
			{
				m_stempSql = m_stempSql + L"[" + m_aFieldNames.GetAt(j) +"]" + " char(255), ";
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += L")";
		}
		...
		{
			// Create new sheet
			m_stempSql.Format(L"CREATE TABLE [%s] (", m_sSheetName);
			for (int i = 0; i < m_aFieldNames.GetSize(); i++)
			{
				m_stempSql = m_stempSql + L"[" + m_aFieldNames.GetAt(i) + L"]" + L" char(255), ";
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += L")";
		}
		...
		for (int k = 1; k < m_dTotalRows; k++)
		{
			...
			// Create Insert SQL
			m_stempSql.Format(L"INSERT INTO [%s$A1:IV%d] (", m_sSheetName, k);
			for (int i = 0; i < m_atempArray.GetSize(); i++)
			{
				m_stempString.Format(L"[%s], ", m_aFieldNames.GetAt(i));
				m_stempSql = m_stempSql + m_stempString;
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += L") VALUES (";
			for (int j = 0; j < m_atempArray.GetSize(); j++)
			{
				m_stempString.Format(L"'%s', ", m_atempArray.GetAt(j));
				m_stempSql = m_stempSql + m_stempString;
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += L")";
	...
	else // if file is a text delimited file
	{
		...
		m_sLastError = L"Error writing file\n";
		return false;
	}
}
...
bool CSpreadSheet::RollBack()
{
	...
	m_sLastError = L"Error in returning to previous state\n";
	return false;
}
...
bool CSpreadSheet::Open()
{
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Code modified ~SJ, 03.07.2013
    // The cell range A1:IV65536 seems to throw errors on Darren's system,
    // so this has been changed to A1:Z30000, which seems to work. No idea why.
    //
		//m_sSql.Format(L"SELECT * FROM [%s$A1:IV65536]", m_sSheetName);
    //
    m_sSql.Format(L"SELECT * FROM [%s$A1:Z30000]", m_sSheetName);
    //
    // End code modified ~SJ, 03.07.2013
    ///////////////////////////////////////////////////////////////////////////
		...
		if (m_dTotalColumns != 0)
		{
			...
			for (int i = 0; i < m_dTotalColumns; i++)
			{
				...
				{
					m_stempString = m_stempString + L"\"" + m_stempSql + L"\"" + m_sSeparator;
				}
				else // Last column
				{	
					m_stempString = m_stempString + L"\"" + m_stempSql + L"\"";
				}				
			}
	...
	else // if file is a text delimited file
	{
		...
		m_sLastError = L"Error in opening file\n";
		return false;
	}
}
...
short CSpreadSheet::CalculateColumnNumber(CString column, bool Auto)
{
	...
	m_sLastError = L"Invalid field name or column alphabet\n";
	return 0;	
}
...
//////////////////////////////////////////////////////////////////////////
// Function name: AddDummyRows
// Description  : Adds dummy rows to the XLS file at the beginning to make
//                believe that the file formats in the columns are TEXT
//////////////////////////////////////////////////////////////////////////
void CSpreadSheet::AddDummyRows()
{
	// Create the Dummy Row
	CStringArray csaRows;
	for (int iCol = 0; iCol < GetTotalColumns(); iCol++) csaRows.Add(L"Dummy");

	// Add the Dummy Row to the XLS
	for (int i = 0; i < 8; i++) AddRow(csaRows, i);
}
